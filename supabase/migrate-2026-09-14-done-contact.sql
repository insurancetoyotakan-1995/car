-- =====================================================================
--  migrate 2026-09-14 (4) : หน้าส่งสำเร็จ — บอกเจ้าหน้าที่ที่ดูแล
--  - แบบ "พนักงานทำประกันเอง" → ins_submit คืน assignee {name, phone} ของเจ้าหน้าที่ที่สุ่มได้
--  - แบบ "แนะนำลูกค้าทั่วไป" → ไม่คืน (ลูกค้าภายนอกกรอกเอง) หน้าเว็บขึ้น "รอติดต่อกลับ"
--  - เพิ่ม ins_staff.phone (ไม่บังคับ) · ใส่แล้วหน้าส่งสำเร็จกดโทรได้
--      update public.ins_staff set phone = '08x-xxx-xxxx' where emp_id = '11001019';
--  วิธีใช้: SQL Editor → วางทั้งไฟล์ → Run (รันซ้ำได้)
-- =====================================================================

alter table public.ins_staff add column if not exists phone text not null default '';

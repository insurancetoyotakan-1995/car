-- ============================================================================
-- วันเริ่มคุ้มครอง ประกันภัย (ภาคสมัครใจ) + พ.ร.บ. (2026-09-19)
--   เจ้าหน้าที่เลือกวันจากปฏิทินในบล็อกเสนอราคา แล้วไปแสดงบนใบคำขอที่พิมพ์
--   (เก็บแค่ "วันเริ่ม" — วันสิ้นสุด = อีก 1 ปี คำนวณตอนแสดงผล)
--
...
-- 🔑 ins_set_quote เพิ่มพารามิเตอร์ 2 ตัว → ต้อง "ลบตัวเก่า" ก่อน
--    (ถ้าปล่อยไว้ทั้งสองตัว PostgREST เลือกไม่ถูกว่าจะเรียกตัวไหน แล้วบันทึกเบี้ยไม่ได้เลย)
-- ============================================================================
...
-- ---------- 3) ins_set_quote รับวันเริ่มคุ้มครอง ----------
...
revoke all on function public.ins_set_quote(uuid, text, text, numeric, numeric, numeric,
                                            numeric, numeric, numeric, text, date, date) from public;
grant execute on function public.ins_set_quote(uuid, text, text, numeric, numeric, numeric,
                                               numeric, numeric, numeric, text, date, date) to authenticated;
...
--            where table_name='ins_requests_view' and column_name in ('quote_start','quote_act_start')) as view_cols,
--          (select count(*) from pg_proc where proname='ins_set_quote') as fn_count;
--   -- ต้องได้ view_cols = 2 · fn_count = 1
-- ============================================================================

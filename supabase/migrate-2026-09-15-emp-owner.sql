-- =====================================================================
--  migrate 2026-09-15 (5) : พนักงานที่เคยทำประกันแล้ว → ได้ผู้ดูแลคนเดิม
--
--  ลำดับการเลือกผู้ดูแล (เฉพาะแบบ "พนักงานทำประกันเอง" + รหัสพนักงานตรงทำเนียบ):
--    1) ใบล่าสุดของพนักงานคนนี้ในระบบนี้ (ไม่นับใบยกเลิก) → เจ้าหน้าที่คนเดิม
--       (ครอบคลุมกรณีบัญชีกลางย้ายผู้ดูแลให้ด้วย — ใบถัดไปตามคนใหม่)
--    2) ตาราง ins_emp_owner (นำเข้าจาก Excel กรมธรรม์เดิม ด้วย supabase/ins-emp-owner.sql)
--    3) ไม่มีทั้งคู่ / เจ้าหน้าที่คนเดิมปิดสิทธิ์หรือเป็นบัญชีผู้ดูแล → แจกคนที่ใบสะสมน้อยสุดเหมือนเดิม
--  แบบ "แนะนำลูกค้า" ไม่ใช้กติกานี้ (ผู้ทำประกันเป็นคนนอก ไม่ใช่ตัวพนักงาน)
--
--  ✅ ไฟล์นี้ต่อยอดจาก migrate-2026-09-15-title.sql (ins_submit รุ่นล่าสุด) — แก้แค่บรรทัดเลือกผู้ดูแล
--  วิธีใช้: SQL Editor → วางทั้งไฟล์ → Run → แล้วรัน ins-emp-owner.sql (ข้อมูลจาก Excel)
--  ✅ รันบน prod แล้ว 2026-09-15 (ใส่ 60 คน · ทดสอบแบบ rollback ผ่านครบ)
--     ⚠️ migrate ไฟล์ใหม่ที่ redefine ins_submit ต้องต่อยอดจากไฟล์นี้ ไม่งั้นกติกาผู้ดูแลคนเดิมหาย
-- =====================================================================

create table if not exists public.ins_emp_owner (
  emp_id        text primary key,          -- รหัสพนักงานที่เป็นผู้ทำประกัน
  agent_emp_id  text not null,             -- เจ้าหน้าที่ประกันผู้ดูแลคนเดิม
  source        text not null default 'excel',
  note          text not null default '',
  updated_at    timestamptz not null default now()
);
alter table public.ins_emp_owner enable row level security;   -- ไม่มี policy = อ่านได้เฉพาะฟังก์ชัน security definer
revoke all on public.ins_emp_owner from anon, authenticated;

create or replace function public.ins_pick_assignee(p_emp text, p_kind text)
returns text
...
begin
  ...
  if p_kind = 'self' and coalesce(p_emp, '') <> '' then
    ...
    select r.assigned_to into v
     ...
     where r.emp_id = p_emp and r.kind = 'self' and r.status <> 'cancelled'
    ...
    select o.agent_emp_id into v
     ...
     where o.emp_id = p_emp;
    if v is not null then return v; end if;
  end if;

  -- 3) ใครมีใบสะสมน้อยสุดได้ก่อน เสมอกันสุ่ม
...
$$;
revoke all on function public.ins_pick_assignee(text, text) from public, anon, authenticated;
...
begin
  ...
  -- คำนำหน้า: นาย / นาง / นางสาว หรือระบุเอง (ไม่เกิน 30 ตัว)
  v_title := left(btrim(coalesce(payload->>'insuredTitle', '')), 30);
  if v_title = '' then
    raise exception 'กรุณาเลือกคำนำหน้าชื่อ' using errcode = 'P0001';
  end if;
  ...
  -- 🐞 lpad ตัดข้อความที่ยาวเกิน: lpad('1000',3) = '100' → ใบที่ 1,000 ของเดือนชนเลขใบที่ 100 (แก้ 2026-09-15)
  v_no := 'INS-' || v_ym || '-' || lpad(v_n::text, greatest(3, length(v_n::text)), '0');

  v_token := encode(gen_random_bytes(16), 'hex');

  -- ผู้ดูแล: พนักงานที่เคยทำประกันแล้วได้คนเดิม (ดู ins_pick_assignee(emp, kind)) · ไม่มีค่อยแจกคนที่ใบน้อยสุด
  v_assignee := public.ins_pick_assignee(case when v_verified then v_emp_id end, v_kind);
